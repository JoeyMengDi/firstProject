...
WORD CEMVBase::GetData(PBYTE pbyTags, CByteStream* pScrData, CByteStream& cRet, unsigned char bFullTlv) {

    DEBUG_ENTRY();

    if ((pbyTags == NULL) || (pScrData == NULL))
    ...
    if (CheckTagNumber(pbyTags) == 2)
        byTag2 = *(pbyTags + 1);

    BYTE *pbySeek, byTempbyte;

    pbySeek = pScrData->GetBuffer();
    PBYTE pEnd = pbySeek + pScrData->GetLength();
    cRet.Empty();
    ...
    while (pHead < pEnd) {
        ...
        if (nTag == 1) {
            ...
            if ((usRet != d_EMVAPLIB_OK) && (usRet != d_EMVAPLIB_ERR_NO_DATA)) {
                ...
                cTemp.Write(pHead, 1);
                DEBUG_STREAM(DEBUGINFO_NORMAL, &cTemp, "Tag ");
            }
        } else if (nTag == 2) {
