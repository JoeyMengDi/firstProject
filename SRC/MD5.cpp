 ...
 *****************************************************************************************/
CByteStream CMd5::GetMD5(const BYTE* pBuf, const UINT &nLength)
{
    CMd5 MD5Checksum;
    MD5Checksum.Update((PBYTE)pBuf, nLength);
    return MD5Checksum.Final();
}

CByteStream CMd5::Encrypt(const CByteStream &cSrcByteStream)
{
    UINT nLength = cSrcByteStream.GetLength();
    PBYTE pBuf = (PBYTE)cSrcByteStream.GetBuffer();
 ...
 *****************************************************************************************/
CByteStream CMd5::Final()
{
    //Save number of bits
    BYTE Bits[8];
    DWordToByte((PBYTE) Bits, (DWORD *) m_nCount, 8);
    ...
    // CString strMD5;
    CByteStream cRetByteStream;
    BYTE byBuffer[3];
    for (int i = 0; i < nMD5Size; i++)
    {
        if (lpszMD5[i] == 0)
        {
            sprintf((char *)byBuffer, "00");
            cRetByteStream.Write(byBuffer, 2);
        }
        else
        {
            sprintf((char *)byBuffer, "%02x", lpszMD5[i]);
            cRetByteStream.Write(byBuffer, 2);
        }
    }
    return cRetByteStream;
}
/*****************************************************************************************
